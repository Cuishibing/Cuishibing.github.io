...
- 首先我认为应该定位到底是哪一个进程占用了过多的CPU   
    ...
    ![](./top.png)
    通过`%Cpu(s)`这一行可以看到当前cpu占用已经达到100%（关于top命令输出的具体含义[看这](/blog/modules/study/linux/1/top命令输出含义.md)），通过下面的列表可以看到占用最多的是一个PID为3792的java进程，毫无疑问是我刚才启动的例子程序，使用top命令很快能找到占用cpu的罪魁祸首。
